...
const api_submit_files = async (post_id: string, files: { name: string, display_name: string, url: string, file: File }[], files_order: number[]) => {
    try {
        const form_data = new FormData();
        for (let i = 0; i < files_order.length; i++) {
            form_data.append(files[files_order[i]].display_name, files[files_order[i]].file);
        }
...
const api_get_distribution_network = async (reload: boolean) => {
    try {
        const response = await fetch('http://' + SERVER_URL + ':' + SERVER_PORT + '/get_distribution_network',
            {
                method: 'POST',
                mode: 'cors',
                headers: {
